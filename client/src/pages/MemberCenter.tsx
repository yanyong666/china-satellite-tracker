...
export default function MemberCenter() {
  ...
  const overviewQuery = trpc.tracker.overview.useQuery(undefined, { refetchInterval: autoRefresh ? 15_000 : false, refetchOnWindowFocus: true });
  const overviewMap = useMemo(() => {
    const map = new Map<string, { price: number; change: number; changePct: number; bars: Array<{ close: number }>; updated: boolean }>();
  ...
  return <div className="member-shell">
    ...
    <main className="member-main">
      ...
      {isSessionLoading ? <MemberStateNotice message="正在读取会员会话" /> : serviceError ? <MemberStateNotice message={serviceError} /> : !hasSession ? <section className="member-auth-card"><div><span className="member-overline">MEMBER ACCESS / NO PAYMENT</span><h2>{mode === "login" ? "登录研究工作台" : "创建你的研究工作台"}</h2><p>不需要银行卡或支付信息。密码只在 Worker 端进行安全哈希，系统不会保存明文密码或证券账户凭据。</p></div><div className="auth-mode"><button className={mode === "login" ? "active" : ""} onClick={() => setMode("login")}>登录</button><button className={mode === "register" ? "active" : ""} onClick={() => setMode("register")}>注册</button></div><form onSubmit={event => void submitAuth(event)}><label>邮箱<input required type="email" value={email} onChange={event => setEmail(event.target.value)} placeholder="researcher@example.com" /></label>{mode === "register" ? <label>显示名称（可选）<input value={displayName} onChange={event => setDisplayName(event.target.value)} maxLength={80} placeholder="研究者" /></label> : null}<label>密码<input required type="password" value={password} onChange={event => setPassword(event.target.value)} minLength={12} maxLength={128} placeholder="至少 12 个字符" /></label>{formError ? <p className="auth-error">{formError}</p> : null}<button className="auth-submit" disabled={register.isPending || login.isPending}>{register.isPending || login.isPending ? <><Loader2 className="animate-spin" size={15} /> 处理中</> : mode === "login" ? "登录会员中心" : "创建无卡会员账户"}<ChevronRight size={15} /></button></form><small className="auth-note">连续错误登录将被暂时限制。注册即表示你同意仅将本服务用于公开研究清单管理。</small></section> : <>
        ...
        <section className="member-saved"><div className="saved-heading"><div><span className="member-overline">SAVED RESEARCH</span><h2>已收藏标的</h2></div><div className="saved-toolbar"><span>{profile?.savedStocks.length ?? 0} 项</span><label className="auto-refresh-toggle"><input type="checkbox" checked={autoRefresh} onChange={(e) => setAutoRefresh(e.target.checked)} /><span>自动刷新 (15s)</span></label></div></div>
          {profile?.savedStocks.length ? <div className="saved-list">{profile.savedStocks.map(entry => {
            ...
            const svgWidth = 110;
            const svgHeight = 32;
            const points = prices.length > 1 ? prices.map((p, i) => {
              const x = (i / (prices.length - 1)) * svgWidth;
              const y = svgHeight - ((p - minPrice) / range) * (svgHeight - 6) - 3;
              return `${x.toFixed(1)},${y.toFixed(1)}`;
            }).join(" ") : "";
            return <article key={entry.stockId}><div className="saved-mark"><Bookmark size={16} /></div><div><small>{entry.stock?.sector ?? "股票池标的"}</small><h3>{entry.stock?.name ?? entry.stockId}</h3><p>{entry.stock?.code ?? entry.stockId}</p></div><div className="saved-sparkline">{prices.length > 1 ? <svg width={svgWidth} height={svgHeight} viewBox={`0 0 ${svgWidth} ${svgHeight}`}><polyline fill="none" stroke={positive ? "#d31e2a" : "#108757"} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" points={points} /></svg> : <span className="sparkline-pending">暂无序列</span>}</div><div className="saved-quote">{quote ? <><strong className={positive ? "up" : "down"}>{quote.price.toFixed(2)}</strong><small className={positive ? "up" : "down"}>{positive ? "+" : ""}{quote.change.toFixed(2)} · {positive ? "+" : ""}{quote.changePct.toFixed(2)}%</small></> : <span className="quote-pending">行情同步中</span>}</div><Link href={`${SITE_ROUTES.terminal}?stock=${entry.stockId}`}>查看终端 <ArrowUpRight size={14} /></Link><button onClick={() => removeStock.mutate({ stockId: entry.stockId as "china-satellite" | "torch-electronics" | "naura" | "zhongji-innolight" | "catl" })}>移除</button></article>;
